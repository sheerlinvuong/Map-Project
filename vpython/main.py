...
def readMPB (root): #reads all multipolygons and puts the ones with the building tag into a list
    mpb = {}        # add all the ways found into a dict
    wd = []
                    ...
                    if rTag == "building":
                        ...
                        for tag in relation.iter('tag'):
                            ...
                            if rTag in ("building:levels", "levels", "level"):
                                v_split = rTag2.split(";")
                                h = max(v_split)
                                mpb[relation_id] = float(h) #sets height if found
                ...
                if rel_tag2 == "building":
                    ...
                    for tag in relation.iter('tag'):       # gets the height
                        ...
                        if rTag in ("building:levels", "levels", "level"):
                            v_split = rTag2.split(";")
                            h = max(v_split)
                            wp[member_ref] = float(h)  # sets height if found
                else:
                    found = 0
                    for member in relation.iter('member'):
                        member_type = member.get('type', default=0)
        ...
        for tag in way.iter('tag'):
            ...
            if wTag == "building":
                ...
                if found != 1:
                    wd.append(i)
                    wp[i] = 1
                    for tag in way.iter('tag'):  # gets the height
                        rTag = tag.get('k', default=0)
                        rTag2 = tag.get('v', default=0)
                        if rTag in ("building:levels", "levels", "level"):
                            v_split = rTag2.split(";")
                            h = max(v_split)
                            wp[i] = float(h)  # sets height if found
    print ("wd2", wd)
    print("wp", wp)
    return wd, wp
...
def drawB (root):
    ...
    for i in wp:
        ...
        if way != "None":
            ...
            s = [(p[0] * scl, -p[1]) for p in vtxo]  # get the x and y components scaled
            poly = Polygon(s)
            extrusion(pos=[(0, 0, 0), (0, wp[i]* 0.00025, 0)], shape=poly, color=color.red)
    return wp
